...
const getMonthlySalary = (emp: Employee): number => {
  ...
  if (emp.salary) {
    ...
    const { basic = 0, hra = 0, conveyance = 0, otherAllowance = 0, specialAllowance = 0, additionalSpecialAllowance = 0 } = salary;
    const total = basic + hra + conveyance + otherAllowance + specialAllowance + additionalSpecialAllowance;
        ...
        onValue(
          ...
          (statusSnap) => {
            ...
            const rows = employees.map((emp) => {
              ...
              const requiredDaysForFull = totalDaysInMonth === 31 ? 27 : 26;
...
const recalculateRow = (row: PayrollRow, emp: Employee, totalDays: number): PayrollRow => {
  ...
  if (baseEarnings > 0 && monthlySalary > 0) {
    // Calculate the ratio of earned vs monthly salary (base only)
    const earningRatio = baseEarnings / monthlySalary;
    ...
    specialAllowance = Number((salaryBreakdown.specialAllowance * earningRatio).toFixed(2));
  }

  // Total gross earnings = base earnings + Additional SP Allowance (fixed, not pro-rated)
  const totalGrossEarnings = baseEarnings + row.additionalSpAllowance;
  ...
  const generatePayslipPDF = async (row: PayrollRow) => {
    ...
    // FIXED INCOME ITEMS - Shows breakdown of total earnings
    const incomeItems = [
                ...
                <Table className="text-xs">
                  <TableHeader>
                    <TableRow className="bg-slate-100 hover:bg-slate-100">
                      ...
                      <TableHead className="font-bold min-w-[100px]">SP All.</TableHead>
                        ...
                        >
                          ...
                          <TableCell className="sticky left-[50px] bg-white z-10">
                            ...
                            <div className="text-[10px] text-gray-500 mt-0.5">
                              Daily Rate: ₹{row.perDayRate.toFixed(2)}
                            </div>
                          </TableCell>
                          <TableCell>
                            {new Date(`${selectedMonth}-01`).toLocaleDateString('en-IN', {
                          ...
                          <TableCell>₹{row.specialAllowance.toLocaleString('en-IN')}</TableCell>
