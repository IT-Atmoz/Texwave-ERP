...
export default function EmployeeForm() {
  ...
  // Check existing portal access record when editing
  useEffect(() => {
    if (!isEdit || !id) return;
    const checkPortal = async () => {
      ...
      if (snap.exists()) {
        ...
        setPortalPassword(snap.val().password || '');
      }
    };
    checkPortal();
  }, [isEdit, id]);
  ...
  useEffect(() => {
    ...
    let special = s.specialAllowance || 0;
    let additionalSpecial = s.additionalSpecialAllowance || 0; // Independent - not limited

    if (conv + special > remaining) {
      const allowedSpecial = Math.max(0, remaining - conv);
      setFormData(prev => ({
        ...prev,
        salary: { ...prev.salary!, specialAllowance: allowedSpecial }
      }));
      return;
    }

    // Other allowance is calculated from remaining (excluding additionalSpecial which is independent)
    const other = remaining - (conv + special);
    // Gross Monthly is the base salary (without Additional SP Allowance)
    const gross = basic + hra + conv + special + other;
    // CTC includes Additional SP Allowance
    ...
    formData.salary?.specialAllowance,
  ...
  const detectSalaryChanges = (): SalaryChange[] => {
    ...
    const allowanceFields = [
      ...
      { key: 'additionalSpecialAllowance', label: 'Additional SP Allowance' },
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isEdit && id) {
        ...
        // Save/update portal access
        if (portalEnabled && portalPassword && formData.email) {
        ...
        // Save portal access for new employee
        if (portalEnabled && portalPassword && formData.email && firebaseKey) {
              ...
              <CardContent className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  <Input
                    ...
                    onChange={(e) => setFormData((p) => ({ ...p, workDomain: e.target.value }))}
                    placeholder="e.g. Textile, IT, Operations"
                  />
                </div>
              </CardContent>
            </Card>
              ...
              <CardContent className="space-y-4">
                <div className="flex items-center space-x-3">
                  <Checkbox
                    ...
                    onCheckedChange={(v) => setPortalEnabled(!!v)}
                  />
                  <Label htmlFor="portal-enabled" className="cursor-pointer font-medium">
                ...
                <CardContent className="space-y-8">
                  ...
                  <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                    <div>
                      ...
                      <Input
                        ...
                        onChange={(e) => setFormData((p) => ({
                          ...p,
                          salary: { ...p.salary!, specialAllowance: Number(e.target.value) || 0 }
                        }))}
                      />
                    </div>

                    <div>
                      <Label>Additional SP Allowance</Label>
                      <Input
                        type="number"
                  ...
                  {grossSalary > 21500 && (
                    <Alert className="bg-amber-50">
                      ...
                      <AlertDescription>
                        ESI/PF not applicable above ₹21,500 gross salary.
                      </AlertDescription>
                    </Alert>
                  )}
