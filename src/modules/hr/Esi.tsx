...
const Esi: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onValue(otRatesRef, (snap) => {
      const data = snap.val() || {};
  ...
  useEffect(() => {
        ...
        employees.forEach((emp) => {
          ...
          empAttendance.forEach((rec) => {
            ...
            if (isSundayDate && rec.status === 'Present') {
              ...
              // For Workers, don't count as present, but add hours to OT
          ...
          const otAmount = (totalOtMinutes / 60) * employeeOtRate;

          // Total gross earnings (SAME AS PAYROLL.TSX)
          const totalGrossEarnings = pdPay + hdPay + holidayPay + sundayPay + otAmount;
    ...
    return () => unsubscribe();
  }, [monthKey, viewMonthKey, employees, activeTab, holidays, otRates]);
